...
const Manager = {
  ...
  update: function (docObject, enrichments) {
    return _.map(enrichments, function (enrichment) {
      let tests = _.map(enrichment.selectors, function (item) {
          return _.map(item.values, function (criterion) {
            return Manager.check(Manager.select(item.selector, docObject), criterion);
          });
        }),
        error = _.reduce(
          tests,
          function (valid, test, key) {
            return (
              valid ||
              test.reduce(function (acc, cur) {
          ...
          return _.map(checks, function (test) {
            if (!test.error)
              return _.map(test.items, function (item) {
                if (!item.error) {
                  if (enrichment.target.from === Manager.from.root)
                    return Manager.setEnrichment([docObject], enrichment);
  ...
  setEnrichment: function (sources, enrichment) {
    if (Array.isArray(sources))
      return _.map(sources, function (source) {
        let _source = Manager.select(enrichment.target.selector, source).source,
          previousValue = _.get(_source, enrichment.target.key, undefined);
        if (!previousValue) _.set(_source, enrichment.target.key, enrichment.value);
        else {
    ...
    if (Array.isArray(enrichment.value)) {
      if (enrichment.erase) {
        ...
        for (let i = 0; i < enrichment.value.length; i++) {
          let flattenPrevious = _.flatten(previous);
          if (Array.isArray(enrichment.value[i])) {
            let flattenValue = _.flatten(enrichment.value[i]),
              duplicates = Manager.sameArrays(flattenPrevious, flattenValue);
            if (duplicates.length === 0) previous.push(enrichment.value[i]);
            else {
              /*
               * TO DO :
               * 1. diff duplicates & flattenValue
               * 2. add each value of diff into previous (How did i get data structure before flatten?)
               */
            }
          } else {
